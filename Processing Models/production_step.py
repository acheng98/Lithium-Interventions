...
class ProductionStep:
	...
	def __init__(self,
			...
			if not isinstance(facility, Facility):
					raise ValueError(f"facility must be a Facility instance, got {type(facility).__name__}")
			...
			# Load in all of the outputs and co-products into the production step.
			...
			# Check to make sure volume_defining_output is in the primary outputs or is a constituent
			...
			# Load in all of the inputs and reagents into the production step.

			# For the input variables, nearly everything should already currently be handled outside of the step
			# and no checks are needed. However, double check this.
			# Check to make sure volume_defining_input is in the primary inputs or is a constituent
	...
	def apply_reagents(self):
			...
			# Update primary output constituents
	...
	def compute_step_pv(self, propagate: bool = False):
			...
			# --- Ratios ---
			output_props = next(iter(self.primary_outputs.values()))
			yield_rate = output_props["yield_rate"]

			if self.volume_defining_output in self.primary_outputs:
					output_ratio = 1.0
			else:
					output_ratio = 1.0  # TODO: extend for intermediates
			...
			if self.volume_defining_input in self.primary_inputs:
					...
					primary_input = next(iter(self.primary_inputs.values()))
	...
	def _calc_labor_costs(self):
			...
			'''
			self.labor_cost = self.facility.wage * self.labor_required * self.facility.plt
	...
	def _calc_capital_costs(self):
			...
			if self.prim_equip_life is not None:
					equip_crf = crf(self.facility.dr, self.prim_equip_life)
			...
			self.maint_cost = self.machine_cost * self.facility.aux_equip
